...
class UniqueColorGenerator:
    ...
    def get_unique_color(self, start_color=None):
        """Get a unique hex color code that hasn't been used before.
        
        Args:
            start_color (str, optional): An optional hex color code to start with.
        
        Returns:
            str: A unique hex color code.
        """
        if start_color and start_color not in self.used_colors:
            self.used_colors.add(start_color)
            return start_color
...
# define a color that won't be used again that
# you use in define_liquid multiple places
# sample_a_color = color_generator.get_unique_color()

# in loops if you want the color unique for each iteration
# call get_unique_color directly
# ctx.define_liquid(name=f"Sample A {n}", description=f"A sample {n}", display_color=color_generator.get_unique_color())
    ...
    for n in range(4):
        if PLATE_TYPE_EACH_PLATE[n] != 0:
            ...
            if TRANSFECTION_TYPE == 1:
                ...
                cell_def = ctx.define_liquid(name="CELL CULTURE PLATE "+str(n+1), description="Cells seeded 24h in advance (culture medium per well)", display_color=color_generator.get_unique_color())  ## Red
            ...
            for mm in range(num_mm): 
                ...
                mm_def = ctx.define_liquid(name="MASTER MIX "+str(mm+1)+" PLATE "+str(n+1), description="DNA and transfection reagent mixture", display_color=color_generator.get_unique_color())  ## Brown
    ...
    if TRANSFECTION_TYPE == 2:
        ...
        for n in range(4):
            if PLATE_TYPE_EACH_PLATE[n] != 0:
                ...
                cell_source_def = ctx.define_liquid(name="CELL SUSPENSION "+str(n+1), description="Cells in culture medium",  display_color=color_generator.get_unique_color())  ## Light Blue
